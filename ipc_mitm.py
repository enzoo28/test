...
LOG_FILE = os.path.join(config.LOG_DIR, f"ipc_mitm_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
...
BRIDGE_EXE = config.BRIDGE_EXE
BRIDGE_DIR = config.BRIDGE_DIR
PROXY_PORT = config.IPC_MITM_PROXY_PORT
BRIDGE_DEFAULT_PORT = config.BRIDGE_DEFAULT_PORT
SCAN_RANGE = range(BRIDGE_DEFAULT_PORT, BRIDGE_DEFAULT_PORT + config.BRIDGE_PORT_SCAN_RANGE)
...
async def mode_listener():
    ...
    bridge_port = await _wait_for_bridge(timeout=config.IPC_MITM_LISTENER_WAIT_TIMEOUT)
    if not bridge_port:
        return
    ...
    try:
        reader, writer = await asyncio.wait_for(
            asyncio.open_connection(config.IPC_MITM_BRIDGE_HOST, bridge_port), timeout=config.IPC_MITM_CONNECT_TIMEOUT
        )
    ...
    try:
        while True:
            chunk = await asyncio.wait_for(reader.read(config.IPC_MITM_READ_SIZE), timeout=config.IPC_MITM_LISTENER_TIMEOUT)
            if not chunk:
                break
            buf += chunk
...
BRIDGE_PORT_FILE = config.BRIDGE_PORT_FILE
...
DEEPCHART_EXE = config.DEEPCHART_EXE
DEEPCHART_DIR = config.DEEPCHART_DIR
...
async def mode_mitm(auto_launch=False):
    ...
    _kill_bridge()
    await asyncio.sleep(config.IPC_MITM_BRIDGE_KILL_DELAY)
    ...
    async def handler(dc_r, dc_w):
        ...
        try:
            b_r, b_w = await asyncio.wait_for(
                asyncio.open_connection(config.IPC_MITM_BRIDGE_HOST, real_bridge_port), timeout=config.IPC_MITM_CONNECT_TIMEOUT
            )
        except Exception as e:
        ...
        async def fwd(r, w, label):
            ...
            try:
                buf = b""
                while True:
                    chunk = await r.read(config.IPC_MITM_READ_SIZE)
                    if not chunk:
                        break
                    n[0] += 1
                    buf += chunk
    ...
    server = await asyncio.start_server(handler, config.IPC_MITM_LISTEN_HOST, PROXY_PORT)
    ...
    real_bridge_port = _wait_for_bridge_sync(timeout=config.IPC_MITM_WAIT_BRIDGE_TIMEOUT)
    if not real_bridge_port:
    ...
    time.sleep(config.IPC_MITM_PORT_FLUSH_DELAY)
...
def _kill_bridge():
    if os.name == "nt":
        subprocess.run(["taskkill", "/F", "/IM", config.BRIDGE_PROCESS_NAME], capture_output=True)
    else:
        subprocess.run(["pkill", "-f", config.BRIDGE_PROCESS_NAME.replace(".exe", "")], capture_output=True)
...
def _find_bridge_port_sync(fast=True) -> int | None:
    ...
    fast=True: only scan first N ports for speed."""
    ports = SCAN_RANGE[:config.BRIDGE_FAST_SCAN_COUNT] if fast else SCAN_RANGE
    for port in ports:
        if port == PROXY_PORT:
            continue
