...
"""
# made by illnoobis
import asyncio, logging, datetime, json, subprocess, zlib, os, sys
import websockets
import config
...
# Try to import CQG protobufs for force-complete responses (moved after log setup)
_PROTOBUF_AVAILABLE = False
_ServerMsg = None
_TimeAndSalesReport = None
...
# Import CQG protobufs after log is set up
for _path in [
    ...
    if os.path.isdir(_path):
        ...
        try:
            from WebAPI.webapi_2_pb2 import ServerMsg as _S
            from WebAPI.historical_2_pb2 import TimeAndSalesReport as _T
            _ServerMsg, _TimeAndSalesReport = _S, _T
            _PROTOBUF_AVAILABLE = True
            log.info(f"[IMPORT] CQG protobufs loaded from: {os.path.abspath(_path)}")
            break
        except ImportError:
...
def _parse_varint(data, pos):
    """Decode a protobuf varint from data starting at pos. Returns (value, new_pos)."""
    value = 0
    shift = 0
    while pos < len(data):
        byte = data[pos]
            ...
            async for message in ws:
                ...
                if isinstance(message, bytes):
                    ...
                    try:
                        ...
                        log.info(f"  [BINARY] Decompressed: {len(decompressed)} bytes | hex={hex_dump(decompressed)}")
